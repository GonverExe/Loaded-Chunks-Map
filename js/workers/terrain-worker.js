 ...
 */
'use strict';
importScripts('nbt-lite.js', 'blockcolors.js', 'biomecolors.js');
...
function readSections(r, itemType) {
    ...
    for (let i = 0; i < n; i++) {
        ...
        for (;;) {
            ...
            else if (k === 'Palette' && t === 9) {
                // 1.13 - 1.17
                r.u8();
                (sec.states || (sec.states = { palette: null, data: null })).palette =
                    readPalette(r);
...
function parseChunk(buf) {
    ...
    const root = (r) => {
        for (;;) {
            ...
            if (k === 'sections' && t === 9) {
                r.u8();
                out.sections = readSections(r);
            } else if (k === 'Sections' && t === 9) {
                r.u8();
                out.sections = readSections(r);
            ...
            else if (k === 'Status' && t === 8) {
                out.full = /full|postprocessed/.test(r.str());
...
function valueAt(data, index, bits, packed) {
    ...
    let li, off;
    if (packed) {
        // 1.16+: valores alineados dentro del long
        const per = (64 / bits) | 0;
        li = (index / per) | 0;
        off = (index % per) * bits;
    } else {
        // 1.13 - 1.15: flujo continuo de bits
    ...
    if (!packed && off + bits > 64) {
        // parte alta en el long siguiente
        ...
        for (let x = 0; x < 16; x++) {
            ...
            for (; y >= minY; y--) {
                ...
                if (!b) {
                    y -= y & 15;
                    continue;
                } // sección vacía: salta entera
                if (b.s.air[b.pi]) continue;

                if (b.s.water[b.pi]) {
                    // superficie de agua: mide el fondo
                    if (!depth) height = y;
                    depth++;
...
function hasTerrain(chunk) {
    ...
    for (const s of chunk.sections) {
        // formatos sin Heightmaps: mirar las paletas
...
function renderChunkBiomes(chunk, pixels, px0, pz0, fixedY) {
    ...
    for (let z = 0; z < 16; z++) {
        for (let x = 0; x < 16; x++) {
            ...
            let s = byY[si];
            if (!s) {
                // sección sin biomas: busca cerca
