...
(function (global) {
    ...
    const STRINGS = {
        es: {
            ...
            'note.simDist':
                'La distancia de simulación no se guarda en el mundo (está en <code>server.properties</code> o en las opciones del cliente). Ajústala a la de tu partida.',
            'spawn.gamerule':
                'Mundo {version}: gamerule <code>spawnChunkRadius = {r}</code> → {side}×{side} chunks con entity ticking.',
            'spawn.classic':
                'Mundo {version}: sin la gamerule <code>spawnChunkRadius</code> (llegó en 1.20.5), así que se aplica el comportamiento clásico, equivalente al radio {r} → 19×19 chunks con entity ticking.',
            'spawn.disabled':
                'Mundo {version}: la gamerule <code>spawnChunkRadius</code> está a 0, así que este mundo no tiene spawn chunks.',
            'spawn.removed':
                'Mundo {version}: los spawn chunks se eliminaron del juego en 1.21.9, así que este mundo no tiene.',
            'spawn.manual': 'Ajustar el radio a mano',
            'spawn.manualNote': 'Radio puesto a mano: {side}×{side} chunks con entity ticking.',
            ...
            'terrain.unavailable':
                'El terreno necesita Web Workers: abre la página desde un servidor (por ejemplo <code>python -m http.server</code>), no con doble clic en el archivo.',
            ...
            'live.start': 'Inicia el mundo',
            'live.stop': 'Detener seguimiento',
            ...
            'live.pick':
                'Al pulsar te pediré la carpeta del mundo para poder releerla mientras juegas.',
            'live.unsupported':
        ...
        en: {
            ...
            'ctl.chunksGrid': '{n} → {side}×{side} chunks',
            'note.simDist':
                'Simulation distance is not stored in the world (it lives in <code>server.properties</code> or in the client options). Set it to the one your game uses.',
            'spawn.gamerule':
                'World {version}: gamerule <code>spawnChunkRadius = {r}</code> → {side}×{side} entity-ticking chunks.',
            'spawn.classic':
                'World {version}: no <code>spawnChunkRadius</code> gamerule (it arrived in 1.20.5), so the classic behaviour applies, equivalent to radius {r} → 19×19 entity-ticking chunks.',
            'spawn.disabled':
                'World {version}: the <code>spawnChunkRadius</code> gamerule is 0, so this world has no spawn chunks.',
            'spawn.removed':
                'World {version}: spawn chunks were removed from the game in 1.21.9, so this world has none.',
            'spawn.manual': 'Set the radius by hand',
            'spawn.manualNote': 'Radius set by hand: {side}×{side} entity-ticking chunks.',
            ...
            'terrain.unavailable':
                'Terrain needs Web Workers: open the page from a server (for instance <code>python -m http.server</code>) instead of double-clicking the file.',
            ...
            'live.pick':
                'Clicking will ask for the world folder so I can re-read it while you play.',
            'live.unsupported':
        ...
        pt: {
            ...
            'ctl.chunksGrid': '{n} → {side}×{side} chunks',
            'note.simDist':
                'A distância de simulação não fica salva no mundo (está no <code>server.properties</code> ou nas opções do cliente). Ajuste para a da sua partida.',
            'spawn.gamerule':
                'Mundo {version}: gamerule <code>spawnChunkRadius = {r}</code> → {side}×{side} chunks com entity ticking.',
            'spawn.classic':
                'Mundo {version}: sem a gamerule <code>spawnChunkRadius</code> (chegou na 1.20.5), então vale o comportamento clássico, equivalente ao raio {r} → 19×19 chunks com entity ticking.',
            'spawn.disabled':
                'Mundo {version}: a gamerule <code>spawnChunkRadius</code> está em 0, então este mundo não tem spawn chunks.',
            'spawn.removed':
                'Mundo {version}: os spawn chunks foram removidos do jogo na 1.21.9, então este mundo não tem.',
            'spawn.manual': 'Ajustar o raio na mão',
            'spawn.manualNote': 'Raio ajustado na mão: {side}×{side} chunks com entity ticking.',
            ...
            'terrain.unavailable':
                'O terreno precisa de Web Workers: abra a página a partir de um servidor (por exemplo <code>python -m http.server</code>), e não com um duplo clique no arquivo.',
            ...
            'live.start': 'Inicia o mundo',
            'live.stop': 'Parar acompanhamento',
            ...
            'live.pick':
                'Ao clicar vou pedir a pasta do mundo para poder relê-la enquanto você joga.',
            'live.unsupported':
        ...
        zh: {
            ...
            'note.simDist':
                '模拟距离不会保存在世界里（它在 <code>server.properties</code> 或客户端设置中）。请调整为你游戏里使用的数值。',
            'spawn.gamerule':
                '世界版本 {version}：游戏规则 <code>spawnChunkRadius = {r}</code> → {side}×{side} 个 entity ticking 区块。',
            'spawn.classic':
                '世界版本 {version}：没有 <code>spawnChunkRadius</code> 游戏规则（1.20.5 才加入），因此采用经典行为，相当于半径 {r} → 19×19 个 entity ticking 区块。',
            'spawn.disabled':
                '世界版本 {version}：<code>spawnChunkRadius</code> 游戏规则为 0，因此这个世界没有出生点区块。',
            'spawn.removed':
                '世界版本 {version}：出生点区块已在 1.21.9 中从游戏里移除，因此这个世界没有。',
            'spawn.manual': '手动设置半径',
            'spawn.manualNote': '手动设置的半径：{side}×{side} 个 entity ticking 区块。',
            ...
            'terrain.unavailable':
                '地形需要 Web Worker：请通过服务器打开页面（例如 <code>python -m http.server</code>），而不是直接双击文件。',
            ...
            'live.pick': '点击后我会请求世界文件夹，以便在你游戏时重新读取它。',
        ...
        ru: {
            ...
            'note.simDist':
                'Дистанция симуляции не хранится в мире (она в <code>server.properties</code> или в настройках клиента). Укажите ту, что используется в вашей игре.',
            'spawn.gamerule':
                'Мир {version}: правило <code>spawnChunkRadius = {r}</code> → {side}×{side} чанков с entity ticking.',
            'spawn.classic':
                'Мир {version}: правила <code>spawnChunkRadius</code> нет (появилось в 1.20.5), поэтому действует классическое поведение, равное радиусу {r} → 19×19 чанков с entity ticking.',
            'spawn.disabled':
                'Мир {version}: правило <code>spawnChunkRadius</code> равно 0, поэтому в этом мире нет спавн-чанков.',
            'spawn.removed':
                'Мир {version}: спавн-чанки убрали из игры в 1.21.9, поэтому в этом мире их нет.',
            'spawn.manual': 'Задать радиус вручную',
            'spawn.manualNote': 'Радиус задан вручную: {side}×{side} чанков с entity ticking.',
            ...
            'terrain.unavailable':
                'Для рельефа нужны Web Workers: откройте страницу через сервер (например <code>python -m http.server</code>), а не двойным щелчком по файлу.',
            ...
            'live.start': 'Запустить мир',
            'live.stop': 'Остановить слежение',
            ...
            'live.pick': 'По нажатию я попрошу папку мира, чтобы перечитывать её, пока вы играете.',
            'live.unsupported':
